...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      setLoading(true);
      const[m,p,c,po,ed]=await Promise.all([dataGet("meta"),dataGet("pesanan"),dataGet("clicks"),dataGet("payouts"),dataGet("extraDebt")]);
  ...
  const metaByDay={};metaData.filter(r=>inDateRange(r["Awal pelaporan"])).forEach(r=>{const d=dateOnly(r["Awal pelaporan"]);if(!d)return;if(!metaByDay[d])metaByDay[d]={spend:0,clicks:0};metaByDay[d].spend+=parseNum(r["Jumlah yang dibelanjakan (IDR)"]);metaByDay[d].clicks+=parseNum(r["Klik tautan"]);});
  ...
  const DATE_FILTERS=[{k:"today",l:"Hari Ini"},{k:"yesterday",l:"Kemarin"},{k:"7d",l:"7 Hari"},{k:"30d",l:"30 Hari"},{k:"all",l:"Semua"}];
  const TABS=[{id:"dashboard",label:"📊 Dashboard"},{id:"pembayaran",label:"💰 Lap. Pembayaran"},{id:"upload",label:"📂 Upload CSV"},{id:"perhari",label:"📆 Per Hari"},{id:"pesanan",label:"🧾 Pesanan"},{id:"kampanye",label:"📣 Kampanye"},{id:"monthly",label:"📅 Bulanan"},{id:"debt",label:"💳 Hutang"}];
    ...
    <div className="app">
      ...
      <main>
        ...
        {tab==="dashboard" && <>
          ...
          {/* Latest payouts */}
          {payouts.slice(0,2).length>0 && <div className="card">
            <div className="ct">Laporan Pembayaran Terakhir</div>
            {payouts.slice(0,2).map(p=><PayoutCard key={p.id} p={p}/>)}
          </div>}
        ...
        {tab==="upload" && <>
          {[
            {key:"meta",title:"📈 Data Meta Ads",icon:"📈",sub:"Export dari Facebook Ads Manager • Breakdown by Day",data:metaData,setData:setMetaData,status:usMeta,setStatus:setUsMeta,handler:handleMeta,label:"Meta Ads"},
            {key:"pesanan",title:"🧾 Laporan Pesanan Affiliate",icon:"🧾",sub:"AffiliateCommissionReport dari Shopee Affiliate",data:pesananData,setData:setPesananData,status:usPesanan,setStatus:setUsPesanan,handler:handlePesanan,label:"Pesanan"},
            {key:"clicks",title:"🖱️ Shopee Clicks",icon:"🖱️",sub:"WebsiteClickReport dari Shopee Affiliate",data:clicksData,setData:setClicksData,status:usClicks,setStatus:setUsClicks,handler:handleClicks,label:"Clicks"},
          ].map(item=>(
            <div className="card" key={item.key}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div className="ct" style={{marginBottom:0}}>{item.title}</div>
                {item.data.length>0&&<button className="btn-d" onClick={async()=>{if(!confirm(`Hapus semua data ${item.label}?`))return;item.setData([]);item.setStatus("");await dataSet(item.key,[]);showToast(`Data ${item.label} dihapus`,"warn");}}>🗑 Hapus ({item.data.length.toLocaleString("id-ID")})</button>}
              </div>
          ...
          <div className="card" style={{borderColor:"#fed7aa"}}>
            ...
            <p style={{fontSize:".8rem",color:"var(--text2)",lineHeight:1.7}}>• Upload berulang aman — duplikat otomatis dilewati<br/>• Gunakan 🗑 Hapus untuk reset data sebelum upload ulang<br/>• Data langsung tersimpan ke Firebase — tim bisa lihat real-time</p>
          </div>
        </>}
        ...
        {tab==="perhari" && <>
          <div className="card">
            <div className="ct">Ringkasan Harian</div>
            ...
            <div className="table-wrap">
              <table className="dt">
                ...
                <tbody>{allDays.map(d=>{
                  ...
                  // CTR Meta→Shopee = Klik Shopee / Klik Meta * 100
                  ...
                  return <tr key={d}>
                    ...
                    <td className="num" style={{color:"var(--yellow)"}}>
                      {cpc!==null?<span title="Cost Per Click Meta">{fmtRp(cpc)}</span>:"-"}
                    </td>
                    <td className="num" style={{color:"var(--blue)"}}>{c>0?c.toLocaleString("id-ID"):"-"}</td>
                    <td className="num">
                      {ctr!==null
                        ? <span className={`badge ${parseFloat(ctr)>=50?"bg-green":parseFloat(ctr)>=20?"bg-yellow":"bg-red"}`}>{ctr}%</span>
                        : "-"}
                    </td>
          ...
          <div className="card" style={{borderColor:"#e0e9ff"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:".78rem",color:"var(--text2)"}}>
              {[["CPC (Cost Per Click)","Biaya per klik iklan Meta = Spend ÷ Klik Meta"],["CTR Meta→Shopee","% klik Meta yang lanjut ke Shopee = Klik Shopee ÷ Klik Meta"],["CTR ≥ 50%","Hijau — konversi klik sangat baik"],["CTR 20–49%","Kuning — cukup, masih bisa dioptimasi"],["CTR < 20%","Merah — perlu evaluasi targeting iklan"]].map(([k,v])=><div key={k} style={{padding:"8px 10px",background:"var(--surface2)",borderRadius:8}}><div style={{fontWeight:700,fontSize:".72rem",marginBottom:3}}>{k}</div><div style={{fontSize:".7rem",color:"var(--text3)"}}>{v}</div></div>)}
            </div>
          </div>
        </>}
