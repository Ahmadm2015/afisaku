...
export default function App() {
  ...
  // Dashboard filters
  const [dateFilter,setDateFilter]=useState("all"); // today|yesterday|7d|30d|all|specific
  ...
  const inDateRange = (iso) => {
    ...
    if(dateFilter==="specific") return specificDate?d===specificDate:true;
    return true;
  };
  ...
  const DATE_FILTERS=[{k:"today",l:"Hari Ini"},{k:"yesterday",l:"Kemarin"},{k:"7d",l:"7 Hari"},{k:"30d",l:"30 Hari"},{k:"specific",l:"📅 Pilih Tanggal"},{k:"all",l:"Semua"}];
        ...
        {tab==="dashboard" && <>
          ...
          <div className="dash-filter">
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
              ...
              {dateFilter==="specific" && (
                <input type="date" value={specificDate} max={today()}
                  onChange={e=>setSpecificDate(e.target.value)}
                  style={{padding:"6px 12px",borderRadius:99,border:"1px solid var(--border)",background:"var(--surface)",color:"var(--text)",fontFamily:"inherit",fontSize:".75rem",fontWeight:600,outline:"none",cursor:"pointer"}}
                />
              )}
          ...
          {/* Active filter label */}
          {dateFilter!=="all" && (
            <div style={{fontSize:".74rem",color:"var(--text3)",fontWeight:600,marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
              <span>📋 Menampilkan:</span>
              <span style={{background:"var(--accent-l)",color:"var(--accent)",padding:"2px 10px",borderRadius:99,fontWeight:700}}>
                {dateFilter==="today"?"Hari Ini ("+today()+")":dateFilter==="yesterday"?"Kemarin ("+daysAgo(1)+")":dateFilter==="7d"?"7 Hari Terakhir":dateFilter==="30d"?"30 Hari Terakhir":dateFilter==="specific"&&specificDate?specificDate:"Pilih tanggal"}
              </span>
              <button onClick={()=>setDateFilter("all")} style={{background:"none",border:"none",cursor:"pointer",color:"var(--text3)",fontSize:".72rem",fontWeight:600}}>✕ Reset</button>
            </div>
          )}
