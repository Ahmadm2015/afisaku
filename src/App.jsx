...
export default function App() {
  ...
  const [tagMappings,setTagMappings]=useState({}); // {tagName: [kampanyeName, ...]}
    ...
    <div className="app">
      ...
      <main>
        ...
        {tab==="perhari" && (()=>{
          ...
          const taxPaid=payouts.filter(p=>p.komisiSebelumPajak>0);
          const avgTaxRate=taxPaid.length>0?taxPaid.reduce((s,p)=>s+(p.potonganPajak/p.komisiSebelumPajak),0)/taxPaid.length:0.025;
          return <>
          <div className="card">
            ...
            <div className="table-wrap">
              <table className="dt">
                ...
                <tbody>{phAllDays.map(d=>{
                  ...
                  const kb=p.komisi*(1-avgTaxRate);
                  return <tr key={d}>
                    ...
                    <td className="num" style={{color:"var(--purple)"}}>{p.komisi>0?fmtRp(kb):"-"}</td>
          ...
          <div className="card" style={{borderColor:"#e0e9ff"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[["CPC","Spend ÷ Klik Meta"],["CTR Meta→Shopee","Klik Shopee ÷ Klik Meta × 100"],["Komisi Kotor","Semua komisi sebelum pajak"],["Komisi Bersih*","Estimasi setelah pajak (~"+(avgTaxRate*100).toFixed(1)+"%)"],["CTR ≥ 50%","🟢 Konversi klik sangat baik"],["CTR < 20%","🔴 Perlu evaluasi targeting"]].map(([k,v])=><div key={k} style={{padding:"8px 10px",background:"var(--surface2)",borderRadius:8,fontSize:".76rem"}}><div style={{fontWeight:700,fontSize:".7rem",marginBottom:3}}>{k}</div><div style={{color:"var(--text3)",fontSize:".68rem"}}>{v}</div></div>)}
            </div>
          </div>
          </>;
        })()}
        ...
        {tab==="tagperforma" && (()=>{
          ...
          const tagStats=allTagsList.map(tag=>{
            const tagPesanan=pesananData.filter(r=>r["Tag_link1"]===tag);
            const tagClicks=clicksData.filter(r=>r["Tag_link"]===tag||r["Tag_link1"]===tag);
            const linkedCampaigns=tagMappings[tag]||[];

            // Meta spend from linked campaigns
            ...
            const komisiSelesai=tagPesanan.filter(r=>r["Status Pesanan"]==="Selesai").reduce((s,r)=>s+parseNum(r["Komisi Bersih Affiliate (Rp)"]),0);
            const pesananCount=tagPesanan.length;
            const selesaiCount=tagPesanan.filter(r=>r["Status Pesanan"]==="Selesai").length;
            const shopeeClicks=tagClicks.length||clicksData.filter(r=>(r["Tag_link"]||"").includes(tag)).length;
            ...
            return {tag,pesananCount,selesaiCount,shopeeClicks,metaClicks,spend,komisi,komisiSelesai,profit,roas,roi,ctr,rateShopee,rateOrder,linkedCampaigns};
          });

          return <>
            <div className="card">
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                <div className="ct" style={{marginBottom:0}}>🏷️ Performa Tag Shopee Affiliate</div>
                <span className="badge bg-orange">{allTagsList.length} tag</span>
              </div>
              <p style={{fontSize:".76rem",color:"var(--text3)",marginBottom:14}}>Kaitkan tag link Shopee ke campaign Meta Ads untuk melihat metrik lengkap per tag.</p>

              {allTagsList.length===0?<div className="empty"><div className="icon">🏷️</div><p>Belum ada data tag.<br/>Upload Laporan Pesanan untuk melihat tag.</p></div>:
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {tagStats.map(ts=>(
                  <div key={ts.tag} style={{background:"var(--surface2)",borderRadius:12,padding:14,border:"1px solid var(--border)"}}>
                    {/* Tag header */}
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                      <div>
                        <div style={{fontWeight:800,fontSize:"1rem",color:"var(--accent)",marginBottom:4}}>#{ts.tag}</div>
                        <div style={{fontSize:".72rem",color:"var(--text3)"}}>{ts.pesananCount} pesanan · {ts.selesaiCount} selesai</div>
                      </div>
                      {ts.roas&&<div style={{textAlign:"right"}}>
                        <div style={{fontFamily:"'DM Mono',monospace",fontWeight:800,fontSize:"1.2rem",color:parseFloat(ts.roas)>=1?"var(--green)":"var(--red)"}}>{ts.roas}x</div>
                        <div style={{fontSize:".65rem",color:"var(--text3)"}}>ROAS</div>
                      </div>}
                    </div>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7,marginBottom:12}}>
                      {[
                        ["Klik Shopee",ts.shopeeClicks.toLocaleString("id-ID"),"var(--blue)"],
                        ["Pesanan",ts.pesananCount,"var(--purple)"],
                        ["Komisi Kotor",fmtRp(ts.komisi),"var(--green)"],
                        ["Ad Spend",ts.spend>0?fmtRp(ts.spend):"-","var(--red)"],
                        ["Profit",ts.profit!==0?fmtRp(ts.profit):"-",ts.profit>=0?"var(--green)":"var(--red)"],
                        ["ROI",ts.roi?ts.roi+"%":"-",parseFloat(ts.roi)>=0?"var(--green)":"var(--red)"],
                      ].map(([l,v,c])=><div key={l} style={{background:"var(--surface)",borderRadius:8,padding:"8px 10px",textAlign:"center"}}>
                        <div style={{fontSize:".62rem",color:"var(--text3)",fontWeight:600,marginBottom:3}}>{l}</div>
                        <div style={{fontWeight:800,fontSize:".85rem",color:c,fontFamily:"'DM Mono',monospace"}}>{v}</div>
                      </div>)}
                    </div>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
                      ...
                      {ts.rateShopee&&<span className="badge bg-blue">Rate Shopee→Order: {ts.rateShopee}%</span>}
                    ...
                    <div style={{borderTop:"1px solid var(--border)",paddingTop:10}}>
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
                        {ts.linkedCampaigns.map(c=>(
                          <span key={c} style={{display:"inline-flex",alignItems:"center",gap:5,background:"var(--blue-l)",color:"var(--blue)",padding:"3px 10px",borderRadius:99,fontSize:".72rem",fontWeight:700}}>
                            {c}
                            <button onClick={async()=>{const nm={...tagMappings,[ts.tag]:ts.linkedCampaigns.filter(x=>x!==c)};await saveTagMappings(nm);}} style={{background:"none",border:"none",cursor:"pointer",color:"var(--red)",fontWeight:800,fontSize:".8rem",padding:0,lineHeight:1}}>×</button>
                          </span>
                        ))}
                        {ts.linkedCampaigns.length===0&&<span style={{fontSize:".72rem",color:"var(--text3)"}}>Belum ada campaign terkait</span>}
                      </div>
                      ...
                      <select
                        ...
                        style={{padding:"6px 10px",background:"var(--surface)",border:"1px solid var(--border2)",borderRadius:8,color:"var(--text2)",fontFamily:"inherit",fontSize:".75rem",outline:"none",cursor:"pointer"}}
                      >
            ...
            {tagStats.length>0&&<div className="card">
              ...
              <div className="table-wrap">
                <table className="dt">
                  ...
                  <tbody>{tagStats.sort((a,b)=>b.komisi-a.komisi).map(ts=>(
                    <tr key={ts.tag}>
                      <td style={{fontWeight:700,color:"var(--accent)"}}>{ts.tag}</td>
