      ...
      <main>
        ...
        {tab==="perhari" && (()=>{
          ...
          metaData.forEach(r=>{const d=dateOnly(r["Awal pelaporan"]);if(!d)return;if(!phMetaByDay[d])phMetaByDay[d]={spend:0,clicks:0};phMetaByDay[d].spend+=parseNum(r["Jumlah yang dibelanjakan (IDR)"]);
            // Klik tautan bisa kosong di export terbaru — fallback ke CTR × Impresi / 100
            const klikLangsung=parseNum(r["Klik tautan"]);
            if(klikLangsung>0){phMetaByDay[d].clicks+=klikLangsung;}
            else{const ctrPct=parseNum(r["CTR (rasio klik tayang tautan)"]);const impresi=parseNum(r["Impresi"]);if(ctrPct>0&&impresi>0)phMetaByDay[d].clicks+=Math.round(ctrPct/100*impresi);}
          });
