...
const parseNum = (v) => {
  ...
  // If already a plain number with dot as decimal (e.g. "45514.45", "1012.2") — just parseFloat
  if (/^\d+(\.\d+)?$/.test(s)) return parseFloat(s) || 0;
  // If uses comma as decimal separator (e.g. "1.234,56") — remove dots, replace comma with dot
  if (/\d\.\d{3},/.test(s) || /,\d{1,2}$/.test(s)) return parseFloat(s.replace(/\./g, "").replace(/,/g, ".")) || 0;
  // If uses dot as thousands separator only (e.g. "1.234") with no decimal — remove dots
        ...
        {tab==="upload" && <>
          ...
          <div className="card">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div className="ct" style={{marginBottom:0}}>📈 Data Meta Ads</div>
              {metaData.length>0 && <button className="btn-d" style={{padding:"4px 10px",fontSize:".72rem",borderRadius:6,border:"1px solid #fca5a5",background:"transparent",color:"var(--red)",cursor:"pointer",fontFamily:"inherit",fontWeight:700}} onClick={async()=>{if(!confirm("Hapus semua data Meta Ads?"))return;setMetaData([]);setUsMeta("");await dataSet("meta",[]);showToast("Data Meta Ads dihapus","warn");}}>🗑 Hapus Data ({metaData.length})</button>}
            </div>
            <UploadZone icon="📈" title="Upload Data Meta Ads (CSV)" sub="Export dari Facebook Ads Manager • Breakdown by Day" onFile={handleMeta} status={usMeta}/>
          </div>

          {/* Pesanan */}
          <div className="card">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div className="ct" style={{marginBottom:0}}>🧾 Laporan Pesanan Affiliate</div>
              {pesananData.length>0 && <button className="btn-d" style={{padding:"4px 10px",fontSize:".72rem",borderRadius:6,border:"1px solid #fca5a5",background:"transparent",color:"var(--red)",cursor:"pointer",fontFamily:"inherit",fontWeight:700}} onClick={async()=>{if(!confirm("Hapus semua data Pesanan?"))return;setPesananData([]);setUsPesanan("");await dataSet("pesanan",[]);showToast("Data Pesanan dihapus","warn");}}>🗑 Hapus Data ({pesananData.length})</button>}
            </div>
            <UploadZone icon="🧾" title="Upload Laporan Pesanan (CSV)" sub="AffiliateCommissionReport dari Shopee Affiliate" onFile={handlePesanan} status={usPesanan}/>
          </div>

          {/* Clicks */}
          <div className="card">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <div className="ct" style={{marginBottom:0}}>🖱️ Shopee Clicks</div>
              {clicksData.length>0 && <button className="btn-d" style={{padding:"4px 10px",fontSize:".72rem",borderRadius:6,border:"1px solid #fca5a5",background:"transparent",color:"var(--red)",cursor:"pointer",fontFamily:"inherit",fontWeight:700}} onClick={async()=>{if(!confirm("Hapus semua data Shopee Clicks?"))return;setClicksData([]);setUsClicks("");await dataSet("clicks",[]);showToast("Data Clicks dihapus","warn");}}>🗑 Hapus Data ({clicksData.length.toLocaleString("id-ID")})</button>}
            </div>
            <UploadZone icon="🖱️" title="Upload Shopee Clicks (CSV)" sub="WebsiteClickReport dari Shopee Affiliate" onFile={handleClicks} status={usClicks}/>
          </div>
          ...
          <div className="card" style={{borderColor:"#fed7aa"}}>
            ...
            <p style={{fontSize:".8rem",color:"var(--text2)",lineHeight:1.7}}>
              ...
              • Tombol 🗑 <strong>Hapus Data</strong> untuk reset tiap jenis data secara terpisah<br/>
