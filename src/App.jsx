      ...
      <main>
        ...
        {tab==="tagperforma" && (()=>{
          ...
          const computeTagStats=(tag)=>{
            ...
            // Clicks pakai format "NamaTag----" — strip trailing dashes untuk match
            const tagClicks=clicksData.filter(r=>(r["Tag_link"]||"").replace(/[-]+$/,"")===tag);
            ...
            // CTR = Klik Shopee / Klik Meta × 100
            const ctr=metaClicks>0?((shopeeClicks/metaClicks)*100).toFixed(1):null;
            // Rate Shopee→Order = Pesanan / Klik Shopee × 100
            ...
            // Daily breakdown — include clicks days too
            ...
            const dailyData=allDaysTag.map(d=>{
              ...
              let dMetaClicks=0;
              tagMeta.filter(r=>dateOnly(r["Awal pelaporan"])===d).forEach(r=>{const kl=parseNum(r["Klik tautan"]);if(kl>0)dMetaClicks+=kl;else{const ct=parseNum(r["CTR (rasio klik tayang tautan)"]);const im=parseNum(r["Impresi"]);if(ct>0&&im>0)dMetaClicks+=Math.round(ct/100*im);}});
              const dKomisi=tagPesanan.filter(r=>dateOnly(r["Waktu Pemesanan"])===d).reduce((s,r)=>s+parseNum(r["Komisi Bersih Affiliate (Rp)"]),0);
              const dShopeeClicks=tagClicks.filter(r=>dateOnly(r["Waktu Klik"])===d).length;
              ...
              const dCtr=dMetaClicks>0?((dShopeeClicks/dMetaClicks)*100).toFixed(1):null;
              return {d,dSpend,dMetaClicks,dKomisi,dShopeeClicks,dPesanan,dRoas,dProfit,dCtr};
            });
          ...
          return <>
              ...
              return (
                <div key={tag} style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:14,padding:16,marginBottom:12,boxShadow:"var(--shadow)"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:14}}>
                    {[
                      ["Klik Meta",metaClicks>0?metaClicks.toLocaleString("id-ID"):"-","var(--yellow)"],
                      ["Klik Shopee",ts.shopeeClicks.toLocaleString("id-ID"),"var(--blue)"],
                      ["Rasio Meta→Shopee",ts.ctr?ts.ctr+"%":"-",parseFloat(ts.ctr)>=50?"var(--green)":parseFloat(ts.ctr)>=20?"var(--yellow)":"var(--red)"],
                      ["Pesanan",ts.pesananCount,"var(--purple)"],
                      ["Komisi Kotor",fmtRp(ts.komisi),"var(--green)"],
                      ["Ad Spend",ts.spend>0?fmtRp(ts.spend):"-","var(--red)"],
                      ["Profit Bersih",ts.spend>0?fmtRp(ts.profit):"-",ts.profit>=0?"var(--green)":"var(--red)"],
                      ...
                      <div key={l} style={{background:"var(--surface2)",borderRadius:9,padding:"9px 10px",textAlign:"center"}}>
                        <div style={{fontSize:".6rem",color:"var(--text3)",fontWeight:600,marginBottom:3}}>{l}</div>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
                    ...
                    {ts.rateShopee&&<span className="badge bg-blue">Klik Shopee→Order: {ts.rateShopee}%</span>}
                      ...
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:".72rem"}}>
                        <thead>
                          <tr style={{borderBottom:"1px solid var(--border)"}}>
                            {["Tanggal","Spend","Klik Meta","Klik Shopee","Rasio CTR","Pesanan","ROAS Hari","Profit Hari"].map(h=><th key={h} style={{padding:"5px 8px",textAlign:h==="Tanggal"?"left":"right",color:"var(--text3)",fontWeight:700,fontSize:".62rem",textTransform:"uppercase",letterSpacing:".4px",whiteSpace:"nowrap"}}>{h}</th>)}
                          </tr>
                        </thead>
                        <tbody>
                          {ts.dailyData.map(({d,dSpend,dMetaClicks,dKomisi,dShopeeClicks,dPesanan,dRoas,dProfit,dCtr})=>(
                            <tr key={d} style={{borderBottom:"1px solid rgba(228,232,242,.5)"}}>
                              ...
                              <td style={{padding:"6px 8px",textAlign:"right",fontFamily:"'DM Mono',monospace",color:"var(--yellow)"}}>{dMetaClicks>0?dMetaClicks.toLocaleString("id-ID"):"-"}</td>
                              <td style={{padding:"6px 8px",textAlign:"right",fontFamily:"'DM Mono',monospace",color:"var(--blue)"}}>{dShopeeClicks>0?dShopeeClicks.toLocaleString("id-ID"):"-"}</td>
                              <td style={{padding:"6px 8px",textAlign:"right"}}>{dCtr?<span style={{fontFamily:"'DM Mono',monospace",fontWeight:700,color:parseFloat(dCtr)>=50?"var(--green)":parseFloat(dCtr)>=20?"var(--yellow)":"var(--red)",fontSize:".7rem"}}>{dCtr}%</span>:"-"}</td>
            ...
            {allTagsList.length>0&&<div className="card">
              ...
              <div className="table-wrap">
                <table className="dt">
                  <thead><tr><th>Tag</th><th>Klik Meta</th><th>Klik Shopee</th><th>Rasio CTR</th><th>Pesanan</th><th>Komisi Kotor</th><th>Spend</th><th>Profit</th><th>ROAS</th></tr></thead>
                  <tbody>{allTagsList.map(tag=>{
                    ...
                    return <tr key={tag} style={{cursor:"pointer"}} onClick={()=>{if(!selectedTags.includes(tag))setSelectedTags(p=>[...p,tag]);}}>
                      ...
                      <td className="num" style={{color:"var(--yellow)"}}>{ts.metaClicks>0?ts.metaClicks.toLocaleString("id-ID"):"-"}</td>
                      <td className="num" style={{color:"var(--blue)"}}>{ts.shopeeClicks>0?ts.shopeeClicks.toLocaleString("id-ID"):"-"}</td>
